...
protected section.
  ...
  methods SEPM_I_SALESOR01_CREATE_ENTITY
    importing
    ...
    exporting
      !ER_ENTITY type ZCL_Z001_MPC=>TS_SEPM_I_SALESORDER_ETYPE
    raising
      /IWBEP/CX_MGW_BUSI_EXCEPTION
      /IWBEP/CX_MGW_TECH_EXCEPTION .
  methods SEPM_I_SALESOR01_DELETE_ENTITY
    importing
      !IV_ENTITY_NAME type STRING
      !IV_ENTITY_SET_NAME type STRING
      !IV_SOURCE_NAME type STRING
    ...
    raising
      /IWBEP/CX_MGW_BUSI_EXCEPTION
      /IWBEP/CX_MGW_TECH_EXCEPTION .
  methods SEPM_I_SALESOR01_GET_ENTITY
    importing
      !IV_ENTITY_NAME type STRING
      !IV_ENTITY_SET_NAME type STRING
      !IV_SOURCE_NAME type STRING
    ...
    raising
      /IWBEP/CX_MGW_BUSI_EXCEPTION
      /IWBEP/CX_MGW_TECH_EXCEPTION .
  methods SEPM_I_SALESOR01_GET_ENTITYSET
    importing
      !IV_ENTITY_NAME type STRING
      !IV_ENTITY_SET_NAME type STRING
      !IV_SOURCE_NAME type STRING
    ...
    exporting
      !ET_ENTITYSET type ZCL_Z001_MPC=>TT_SEPM_I_SALESORDER_ETYPE
      !ES_RESPONSE_CONTEXT type /IWBEP/IF_MGW_APPL_SRV_RUNTIME=>TY_S_MGW_RESPONSE_CONTEXT
    raising
      /IWBEP/CX_MGW_BUSI_EXCEPTION
      /IWBEP/CX_MGW_TECH_EXCEPTION .
  methods SEPM_I_SALESOR01_UPDATE_ENTITY
    importing
      !IV_ENTITY_NAME type STRING
      !IV_ENTITY_SET_NAME type STRING
      !IV_SOURCE_NAME type STRING
  ...
  methods SEPM_I_SALESORDE_CREATE_ENTITY
    importing
      ...
      !IO_DATA_PROVIDER type ref to /IWBEP/IF_MGW_ENTRY_PROVIDER optional
    exporting
      !ER_ENTITY type ZCL_Z001_MPC=>TS_SEPM_I_SALESORDERITEM_ETYPE
    raising
      /IWBEP/CX_MGW_BUSI_EXCEPTION
      /IWBEP/CX_MGW_TECH_EXCEPTION .
  ...
  methods SEPM_I_SALESORDE_GET_ENTITYSET
    importing
      ...
      !IO_TECH_REQUEST_CONTEXT type ref to /IWBEP/IF_MGW_REQ_ENTITYSET optional
    exporting
      !ET_ENTITYSET type ZCL_Z001_MPC=>TT_SEPM_I_SALESORDERITEM_ETYPE
      !ES_RESPONSE_CONTEXT type /IWBEP/IF_MGW_APPL_SRV_RUNTIME=>TY_S_MGW_RESPONSE_CONTEXT
    raising
      /IWBEP/CX_MGW_BUSI_EXCEPTION
      /IWBEP/CX_MGW_TECH_EXCEPTION .
...
CLASS ZCL_Z001_DPC IMPLEMENTATION.
...
*&* This class has been generated on 04.07.2017 12:36:34 in client 001
*&*
*&*       WARNING--> NEVER MODIFY THIS CLASS <--WARNING
*&*   If you want to change the DPC implementation, use the
*&*   generated methods inside the DPC provider subclass - ZCL_Z001_DPC_EXT
*&-----------------------------------------------------------------------------------------------*

 DATA sepm_i_salesorde_create_entity TYPE zcl_z001_mpc=>ts_sepm_i_salesorderitem_etype.
 DATA sepm_i_salesor01_create_entity TYPE zcl_z001_mpc=>ts_sepm_i_salesorder_etype.
...
*-------------------------------------------------------------------------*
*             EntitySet -  SEPM_I_SalesOrder_E
*-------------------------------------------------------------------------*
     WHEN 'SEPM_I_SalesOrder_E'.
*     Call the entity set generated method
    sepm_i_salesor01_create_entity(
         EXPORTING iv_entity_name     = iv_entity_name
                   iv_entity_set_name = iv_entity_set_name
                   iv_source_name     = iv_source_name
                   io_data_provider   = io_data_provider
                   ...
                   it_navigation_path = it_navigation_path
                   io_tech_request_context = io_tech_request_context
       	 IMPORTING er_entity          = sepm_i_salesor01_create_entity
    ).
*     Send specific entity data to the caller interfaces
    copy_data_to_ref(
      EXPORTING
        is_data = sepm_i_salesor01_create_entity
      CHANGING
        cr_data = er_entity
   ).
...
*-------------------------------------------------------------------------*
      when 'SEPM_I_SalesOrder_E'.
*     Call the entity set generated method
     sepm_i_salesor01_delete_entity(
          EXPORTING iv_entity_name     = iv_entity_name
                    iv_entity_set_name = iv_entity_set_name
                    iv_source_name     = iv_source_name
                    it_key_tab         = it_key_tab
...
*&-----------------------------------------------------------------------------------------------*
 ...
 DATA sepm_i_salesorde_get_entity TYPE zcl_z001_mpc=>ts_sepm_i_salesorderitem_etype.
 DATA sepm_i_salesor01_get_entity TYPE zcl_z001_mpc=>ts_sepm_i_salesorder_etype.
 DATA lv_entityset_name TYPE string.
...
*     Call the entity set generated method
          sepm_i_salesor01_get_entity(
               EXPORTING iv_entity_name     = iv_entity_name
                         ...
                         it_navigation_path = it_navigation_path
                         io_tech_request_context = io_tech_request_context
             	 IMPORTING er_entity          = sepm_i_salesor01_get_entity
                         es_response_context = es_response_context
          ).

        IF sepm_i_salesor01_get_entity IS NOT INITIAL.
*     Send specific entity data to the caller interface
          copy_data_to_ref(
            EXPORTING
              is_data = sepm_i_salesor01_get_entity
            CHANGING
              cr_data = er_entity
          ).
        ELSE.
...
*&*   generated methods inside the DPC provider subclass - ZCL_Z001_DPC_EXT
*&-----------------------------------------------------------------------------------------------*
 DATA sepm_i_salesorde_get_entityset TYPE zcl_z001_mpc=>tt_sepm_i_salesorderitem_etype.
 DATA sepm_i_salesor01_get_entityset TYPE zcl_z001_mpc=>tt_sepm_i_salesorder_etype.
 DATA lv_entityset_name TYPE string.
...
*-------------------------------------------------------------------------*
   WHEN 'SEPM_I_SalesOrder_E'.
*     Call the entity set generated method
      sepm_i_salesor01_get_entityset(
        EXPORTING
         iv_entity_name = iv_entity_name
         iv_entity_set_name = iv_entity_set_name
         iv_source_name = iv_source_name
         ...
         io_tech_request_context = io_tech_request_context
       IMPORTING
         et_entityset = sepm_i_salesor01_get_entityset
         es_response_context = es_response_context
       ).
*     Send specific entity data to the caller interface
      copy_data_to_ref(
        EXPORTING
          is_data = sepm_i_salesor01_get_entityset
        CHANGING
          cr_data = er_entityset
      ).
...
*&-----------------------------------------------------------------------------------------------*
 ...
 DATA sepm_i_salesorde_update_entity TYPE zcl_z001_mpc=>ts_sepm_i_salesorderitem_etype.
 DATA sepm_i_salesor01_update_entity TYPE zcl_z001_mpc=>ts_sepm_i_salesorder_etype.
 DATA lv_entityset_name TYPE string.
 DATA lr_entity TYPE REF TO data. "#EC NEEDED
...
*-------------------------------------------------------------------------*
      WHEN 'SEPM_I_SalesOrder_E'.
*     Call the entity set generated method
          sepm_i_salesor01_update_entity(
               EXPORTING iv_entity_name     = iv_entity_name
                         iv_entity_set_name = iv_entity_set_name
                         iv_source_name     = iv_source_name
                         io_data_provider   = io_data_provider
                         ...
                         it_navigation_path = it_navigation_path
                         io_tech_request_context = io_tech_request_context
             	 IMPORTING er_entity          = sepm_i_salesor01_update_entity
          ).
       IF sepm_i_salesor01_update_entity IS NOT INITIAL.
*     Send specific entity data to the caller interface
          copy_data_to_ref(
            EXPORTING
              is_data = sepm_i_salesor01_update_entity
            CHANGING
              cr_data = er_entity
          ).
        ELSE.
...
* Unit test injection
  ...
  method IF_SADL_GW_DPC_UTIL~GET_DPC.
    TYPES ty_sepm_i_salesorderitemte_1 TYPE sepm_i_salesorderitemtext_e ##NEEDED. " reference for where-used list
    TYPES ty_sepm_i_salesorderitem_e_2 TYPE sepm_i_salesorderitem_e ##NEEDED. " reference for where-used list
    TYPES ty_sepm_i_salesordertext_e_3 TYPE sepm_i_salesordertext_e ##NEEDED. " reference for where-used list
    TYPES ty_sepm_i_salesorder_e_4 TYPE sepm_i_salesorder_e ##NEEDED. " reference for where-used list
    ...
    DATA(lv_sadl_xml) =
               ...
               | <sadl:query name="SADL_QUERY">| &
               | </sadl:query>| &
               | <sadl:association name="TO_ITEM" binding="_ITEM" target="SEPM_I_SalesOrderItem_E" cardinality="many" />| &
               |</sadl:structure>| &
               |</sadl:resultSet>| &
               |</sadl:definition>| .
    ro_dpc = cl_sadl_gw_dpc_factory=>create_for_sadl( iv_sadl_xml   = lv_sadl_xml
               iv_timestamp         = 20170704123626
               iv_uuid              = 'Z001'
               io_context           = me->mo_context ).
  endmethod.


  method SEPM_I_SALESOR01_CREATE_ENTITY.
  ...
  method SEPM_I_SALESOR01_UPDATE_ENTITY.
